...
HTTP transport variant — runs as a standalone network service so
multiple independent agent processes (possibly on different machines)
can connect to the same server over HTTP instead of one client
spawning it as a local subprocess.

Run with:
    python server_http.py

Then point any number of MCP clients at:
    http://<host>:8000/mcp
"""
...
mcp = FastMCP(
    ...
    stateless_http=True,  # each request is independent; no server-side session pinning needed
)
store = MemoryStore()


# --- Tool definitions are IDENTICAL to server.py, unchanged ---
